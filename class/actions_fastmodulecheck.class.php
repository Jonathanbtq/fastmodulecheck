		...
		if (in_array('toprightmenu', $contexts)) {
			...
			if ($mode == 'common' || $mode == 'commonkanban') {
				foreach ($orders as $key => $value) {
					...
					if (getDolGlobalString($const_name) && (getDolGlobalInt('MAIN_FEATURES_LEVEL') < 2)) {
						$disableSetup = 0;
						...
						if (!empty($objMod->disabled)) {
							...
							$codeenabledisable .= '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$objMod->numero.'&amp;token='.newToken().'&amp;module_position='.$module_position.'&amp;action=reset&amp;value='.$modName.'&amp;mode='.$mode.'&amp;confirm=yes'.$param.'">';
							$codeenabledisable .= img_picto($langs->trans("Enabled"), 'switch_on');
							$codeenabledisable .= '</a>';
						}
					} else {
						// MODULE PAS ACTIVER
						$codeenabledisable .= '<a class="reposition valignmiddle" href="'.$_SERVER["PHP_SELF"].'?id='.$objMod->numero.'&token='.newToken().'&module_position='.$module_position.'&action=set&token='.newToken().'&value='.$modName.'&mode='.$mode.'&amp;confirm=yes'.$param.'"';
						$codeenabledisable .= '>';
